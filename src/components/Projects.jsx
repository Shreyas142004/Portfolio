...
const projectsData = [
  ...
  },
  {
    ...
    shortDescription:
      "Modern responsive frontend for a smart utility application.",
    fullDescription:
      "A fully responsive and interactive frontend dashboard built for a smart utility system. Developed using React and styled with Tailwind CSS, it features a modern, clean UI designed to display complex utility metrics clearly. Focuses heavily on component reusability and sleek design aesthetics.",
  ...
  },
  {
    ...
    image: "./speech.png",
    shortDescription:
      "High-fidelity speech transcription engine online.",
    fullDescription:
  ...
  },
  {
    ...
    shortDescription:
      "A full-stack web application for seamless, real-time parking space reservations.",
    fullDescription:
      "A responsive full-stack platform that allows users to view available parking locations, manage bookings, and handle secure reservations. Built with React and Tailwind CSS for the frontend, integrated with a Node.js/Express backend, JWT authentication, and MongoDB for scalable data storage.",
    tech: [
      "React",
      "Node.js",
      "Express.js",
  ...
  return (
    ...
    >
      <motion.div
        ...
        whileHover={
          window.matchMedia("(hover: hover)").matches ? { scale: 1.03 } : {}
        }
      ...
      >
        ...
        <div className="relative h-48 overflow-hidden lg:transform-[translateZ(30px)]">
          <div
            className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10 mix-blend-overlay ${isDark ? "bg-neon-cyan/20" : "bg-blue-500/20"}`}
          />
          <img
            src={project.image}
            alt={project.title}
        ...
        {/* Content */}
        <div
          className={`p-6 relative z-20 backdrop-blur-sm flex-1 flex flex-col lg:transform-[translateZ(40px)] ${isDark ? "bg-black/80" : "bg-white/90"}`}
        >
          <p
            className={`font-rajdhani text-sm font-bold tracking-widest mb-1 ${isDark ? "text-neon-purple" : "text-pink-600"}`}
          >
            {project.category}
          </p>
          <h3
            className={`text-xl font-orbitron font-bold mb-3 transition-all ${isDark ? "text-white group-hover:text-glow" : "text-gray-900 group-hover:text-blue-600"}`}
          >
            {project.title}
          </h3>

          <p
            className={`font-rajdhani text-sm mb-6 flex-1 ${isDark ? "text-gray-400" : "text-gray-600"}`}
          >
          ...
          <div className="flex justify-between items-center mt-auto">
            <span
              className={`text-xs font-orbitron font-bold uppercase tracking-wider ${isDark ? "text-neon-cyan" : "text-blue-600"}`}
            >
              Click for details
            </span>
        ...
        <div
          className={`absolute bottom-0 left-0 h-1 w-0 group-hover:w-full transition-all duration-500 z-20 lg:transform-[translateZ(50px)] ${isDark ? "bg-neon-cyan shadow-[0_0_10px_#00ffff]" : "bg-blue-600"}`}
        />
      </motion.div>
    </motion.div>
  );
...
const Projects = () => {
  ...
  return (
    <section
      id="projects"
      className="relative bg-transparent py-24 min-h-screen overflow-hidden"
    >
      {/* Background Decor */}
      <div
        className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full h-150 blur-[150px] pointer-events-none ${isDark ? "bg-neon-purple/5" : "bg-blue-300/10"}`}
      />
      ...
      <div className="z-10 relative mx-auto px-4 sm:px-6 lg:px-8 pt-10 max-w-7xl">
        ...
        >
          <h2
            className={`text-4xl md:text-5xl font-orbitron font-bold mb-4 ${isDark ? "text-white" : "text-gray-900"}`}
          >
          ...
          <div
            className={`w-24 h-1 mx-auto ${isDark ? "bg-neon-purple box-glow" : "bg-pink-500"}`}
          />
        </motion.div>
          ...
          >
            <motion.div
              ...
              className={`max-w-xl w-full max-h-[90vh] overflow-y-auto overflow-x-hidden rounded-2xl border relative ${
                isDark
                  ? "glass-panel border-neon-cyan/40 box-glow"
            ...
            >
              <button
                ...
                className={`absolute top-4 right-4 z-50 p-2 rounded-full backdrop-blur-sm transition-colors ${
                  isDark
                    ? "text-white/50 hover:text-neon-cyan bg-black/50"
                    : "text-gray-500 hover:text-blue-600 bg-white/80"
                }`}
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="relative h-48 md:h-56">
                ...
                />
                <div
                  className={`absolute inset-0 bg-linear-to-t ${isDark ? "from-black via-black/50" : "from-white via-white/50"} to-transparent`}
                />
                <div className="right-5 bottom-5 left-5 absolute">
                  <p
                    className={`font-rajdhani font-bold tracking-widest mb-1 ${isDark ? "text-neon-cyan" : "text-blue-600"}`}
                  >
                    {selectedProject.category}
                  </p>
                  <h3
                    className={`text-2xl md:text-3xl font-orbitron font-bold ${isDark ? "text-white text-glow" : "text-gray-900"}`}
                  >
              ...
              <div
                className={`p-5 md:p-6 ${isDark ? "bg-black/80" : "bg-white"}`}
              >
                <p
                  className={`font-rajdhani text-base leading-relaxed mb-5 ${isDark ? "text-gray-300" : "text-gray-700"}`}
                >
                ...
                <div className="flex flex-wrap gap-2 mb-8">
                  {selectedProject.tech.map((t) => (
                    <span
                      key={t}
                      className={`px-3 py-1 text-sm font-orbitron rounded border ${
                        isDark
                ...
                <div className="flex flex-wrap gap-4">
                  {selectedProject.github !== "#" ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className={`flex items-center gap-2 px-6 py-3 font-orbitron font-bold rounded border transition-all ${
                        isDark
                    ...
                    <div
                      className={`flex items-center gap-2 px-6 py-3 font-orbitron font-bold rounded border cursor-not-allowed opacity-50 ${
                        isDark
